...
```javascript
var companies = ["Google", "Facebook", "ABB"];
...
function signUp(options) {
  // set needed arguments
  console.log(
    options.username + options.password + options.email + options.city
  );
}
var user = {
  username: "myname",
...
function signUp({ username, password, email, city }) {
  // take those properties from object and we can put arguments in different order
